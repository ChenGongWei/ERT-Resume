...
export default class MyInput extends React.PureComponent<InputProps, InputState> {
  ...
  renderBefore() {
    ...
    return !!addonBefore && <div className={less["my-input-center"]}>{addonBefore}</div>;
  }

  renderAfter() {
    const { addonAfter } = this.props;
    return !!addonAfter && <div className={less["my-input-center"]}>{addonAfter}</div>;
  }

  renderClear() {
    const { allowClear } = this.props;
    return !!allowClear && this.state.text && <i className={less["my-input-clear"]} onClick={this.onClear} />;
  }
  ...
  renderInput() {
    ...
    return (
      <div
        className={classnames(less[`my-input-input`], less[`${size}`])}
      >
        <input
  ...
  renderTextarea() {
    ...
    return (
      <div className={less["my-input-textarea"]} style={{ height: 24 * _rows }}>
        <textarea
          {...{ placeholder, maxLength, id, disabled, autoFocus }}
          rows={_rows}
          value={text}
        ...
        {this.renderClear()}
        {allowCount && (
          <div className={less["my-input-textarea-footer"]}>
            <span
              className={classnames(less[!!maxLength && text && String(text).length >= maxLength ? 'max-length-text' : ''])}
            >
              {String(text).length}
            </span>
            {!!maxLength && (
  ...
  render() {
    ...
    return (
      <div
        style={style}
        className={classnames(less['my-input'], 
          less[!bgTransparent ? 'normal' : ''],
          less[this.state.focus ? 'focus' : '' ],
          less[allowClear ? 'allow-clear' : '' ],
        )}
      >
        {this.renderBefore()}
